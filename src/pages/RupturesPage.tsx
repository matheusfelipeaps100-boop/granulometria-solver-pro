...
import type { ScheduleStatus } from "@/store/useAppStore";
...
const RupturesPage = () => {
  ...
  // Batch-level status
  const batchStatus = (schedules: typeof allSchedules) => {
    if (schedules.every((s) => s.status === "concluido")) return "concluido";
    if (schedules.some((s) => s.status === "atrasado")) return "atrasado";
    if (schedules.some((s) => s.status === "em_andamento")) return "em_andamento";
    return "pendente";
  };
            ...
            <Table>
              ...
              <TableBody>
                {filtered.map(({ batch, analysis, schedules }) => {
                  const overallStatus = batchStatus(schedules);
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          {sorted.map((s) => (
                            <button
                              key={s.id}
                              onClick={() => navigate(`/ruptures/${s.id}`)}
                              className={`inline-flex items-center justify-center rounded-md border px-2.5 py-0.5 text-xs font-semibold transition-colors cursor-pointer ${statusColor[s.status]}`}
                              title={`${s.idade_dias}d — ${s.status} — ${formatDate(s.data_prevista)}`}
                            >
                              {s.idade_dias}d
                            </button>
                          ))}
