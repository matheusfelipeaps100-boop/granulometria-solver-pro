...
// Peneiras onde a faixa ±10pp é "clampada" em 0%/100% (0,15mm e Fundo) —
// o centro da faixa não fica exatamente igual à referência nessas duas,
// por construção; nas demais 7 peneiras, sim.
const SIEVE_IDS_CLAMPADOS = [9, 10];

describe("LIMITES_LAJE_PADRAO — curva de referência ±10pp (traço real Concreart)", () => {
  ...
  it("a faixa tem exatamente ±10 pontos percentuais (exceto onde clampado em 0%/100%)", () => {
    for (const limite of LIMITES_LAJE_PADRAO) {
      if (limite.sieve_id === 1) continue;
      const largura = limite.limite_max - limite.limite_min;
      expect(largura).toBeLessThanOrEqual(0.2 + 1e-9);
    }
  });
});
